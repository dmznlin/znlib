{ Unit Main
  ...
  28 May 2007: Made compatible with sdFastXml

    Properties of TXmlNode
    - AttributeByName -> AttributeValueByName
    - ValueAsWidestring -> ValueWide

    Properties/methods of TsdFastXml compared to TNativeXml
    - StyleSheetNode -> StyleSheet
    - Utf8Encoded no longer exists (is always the case)
    - OnUnicodeLoss no longer exists (user should know that when reading
        ascii, characters above #127 are converted to question marks)
    - Xmlformat -> OutputFormatting
    - TsdFastXml is now component, so Create has argument AOwner
  ...
  copyright (c) 2001 - 2007 Nils Haeck  www.simdesign.nl
...
type
  ...
  private
    FXmlDoc: TsdFastXml;    // Xml document currently displayed
...
const
  ...
  cAppVersion        = 'v2.1';
  cFormHeader        = 'Xml Editor (c) 2001-2007 SimDesign B.V.';
...
begin
  ...
  begin
    ...
    try
      ...
      sbMain.SimpleText := Format('Version="%s"', [FXmlDoc.VersionString]);
      if Length(FXmlDoc.EncodingString) > 0 then
        sbMain.SimpleText := sbMain.SimpleText +
          Format(' Encoding="%s"', [FXmlDoc.EncodingString]);
      sbMain.SimpleText := sbMain.SimpleText +
        Format('unique strings: %d', [FXmlDoc.FStrings.StringCount]);
    except
      // Show exception on status bar
      on E: Exception do
        sbMain.SimpleText := Format('%s pos:%d line:%d',
          [E.Message, FXmlDoc.ReaderPosition, FXmlDoc.ReaderLineNumber]);
    end;
    Regenerate;
  end;
end;
...
begin
  if IsUpdating then
    exit;
  // switch them around
  case FXmlDoc.OutputFormatting of
  xfReadable: FXmlDoc.OutputFormatting := xfCompact;
  xfCompact:  FXmlDoc.OutputFormatting := xfReadable;
  end;
  UpdateMenu;
end;
...
begin
  FXmlDoc := TsdFastXml.Create(Self);
...
begin
  ...
  begin
    ...
    1: CellText := TXmlNode.ToWide(FFocusedNode.AttributeValue[Index]);
    end;//case
  end else
  begin
...
begin
  ...
  begin
    ...
    1: FFocusedNode.AttributeValue[Index] := TXmlNode.FromWide(NewText);
    end;//case
  end else
  begin
    ANode := FFocusedNode.Nodes[Index];
...
var
  OldFormat: TsdFormattingType;
begin
  if (pcTree.ActivePage = tsXmlSource) then
  begin
    // Show our Xml Document in readable format
    Oldformat := FXmlDoc.OutputFormatting;
    try
      FXmlDoc.OutputFormatting := xfReadable;
      if FXmlDoc.IsEmpty then
        mmXmlSource.Text := ''
      else
        mmXMLSource.Text := FXmlDoc.SaveToString;
    finally
      FXmlDoc.OutputFormatting := OldFormat;
    end;
  end;
end;
...
begin
  ...
  1: FData.FNode.ValueWide := NewText;
  end;//case
end;
...
begin
  ...
  begin
    ...
    1: CellText := FData^.FNode.ValueWide;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      FData := Sender.GetNodeData(Node);
      FData^.FNode := FXmlDoc.RootNodes[Node.Index];
      InitialStates := [];
      if assigned(FData^.FNode) then
      begin
...
begin
  // Redraw XML tree
  stXmlTree.Clear;
  stXmlTree.RootNodeCount := FXmlDoc.RootNodes.Count;
  // Properties pane
  RegenerateProperties;
  // Form caption
...
begin
  BeginUpdate;
  try
    acOutputReadable.Checked := FXmlDoc.OutputFormatting = xfReadable;
  finally
     EndUpdate;
  end;
end;
...
begin
  ...
  ANode := TsdElement.Create(FXmlDoc);
  ANode.Name := 'element';
...
begin
  ...
  if assigned(AParent) then
  begin
    ANode := TsdComment.Create(FXmlDoc);
    AParent.NodeInsert(AParent.NodeIndexOf(FFocusedNode), ANode);
    RegenerateFromNode(AParent);
  end;
end;
