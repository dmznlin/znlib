...
type
  ...
  public
    ...
    property SourceCode: widestring read GetSourceCode write SetSourceCode;
  end;
...
begin
  ...
  begin
    Inc(FOffset);
    while FOffset < FCompressedRecordEnd do
    begin
...
begin
  ...
  begin
    ...
    if FCompressedEnd > FCompressedRecordEnd then
      FCompressedEnd := FCompressedRecordEnd;
    Inc(FOffset, 2);
    if CompressedFlag then
    begin
      while FOffset < FCompressedEnd do
      begin
        Result := DecompressTokenSequence()
      end;
      if FData.DataLength > 0 then
      begin
...
begin
  ...
  Inc(FOffset);
  if FOffset < FCompressedEnd then
  begin
    for i := 0 to 7 do
    begin
      if FOffset < FCompressedEnd then
...
begin
  Result := nil;
  if Assigned(FVBAStorage) then
  begin
    r := FVBAStorage.GetChildByName('vba');
    if Assigned(r) then
    begin
      if r is TXLSFileStorage then
      begin
...
begin
  Result := nil;
  if Assigned(FVBA) then
  begin
    r := FVBA.GetChildByName('_vba_project');
    if Assigned(r) then
    begin
      if r is TXLSFileStream then
      begin
...
procedure TXLSVBAModule.SetSourceCode(const Value: widestring);
begin
  FSourceCode := Value;
end;
