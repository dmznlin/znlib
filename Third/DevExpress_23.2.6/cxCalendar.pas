...
uses
  ...
  SysUtils, Classes, Graphics, Controls, Forms, StdCtrls, Clipbrd, RTLConsts, Generics.Defaults, Generics.Collections,
  ...
  dxFading, dxDateTimeWheelPicker, dxTypeHelpers, cxAccessibility;
...
{ TcxMonthListBox - Bug in Delphi5: Must be in the interface part }
  ...
  { TdxCalendarElementAccessibilityHelper }

  TdxCalendarElementAccessibilityHelper = class(TcxAccessibilityHelper)
  strict private
    FIAccessibilityChildren: TInterfaceList;
    FParent: TcxAccessibilityHelper;

    function GetViewInfo: TcxCalendarElementViewInfo;
  protected
    FLocalId: Integer;
    ...
    property ViewInfo: TcxCalendarElementViewInfo read GetViewInfo;
  public
    constructor Create(AOwnerObject: TObject; AParent: TcxAccessibilityHelper); reintroduce; virtual;
    destructor Destroy; override;

    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  { TdxCalendarButtonAccessibilityHelper }

  TdxCalendarButtonAccessibilityHelper = class(TdxCalendarElementAccessibilityHelper)
  protected
  ...
  TdxCalendarElementAccessibilityHelperClass = class of TdxCalendarElementAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass; virtual;
    ...
    function GetIAccessibilityHelper: IcxAccessibilityHelper; virtual;
    ...
    procedure PopulateIAccessibilityChildren(IAccessibilityChildren: TInterfaceList); virtual;
  ...
  { TdxCalendarArrowAccessibilityHelper }

  TdxCalendarArrowAccessibilityHelper = class(TdxCalendarButtonAccessibilityHelper)
  protected
    procedure DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID); override;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass; override;
  ...
  { TdxCalendarDateCellSelectionItemProvider }

  TcxCalendarDateCellViewInfo = class;

  TdxCalendarDateCellSelectionItemProvider = class(TdxSelectionItemProvider)
  strict private
    FViewInfo: TcxCalendarDateCellViewInfo;
  protected
    procedure DoSelect; override;
    function IsSelected: Boolean; override;
  public
    constructor Create(AViewInfo: TcxCalendarDateCellViewInfo);
  end;

  { TdxCalendarDateCellAccessibilityHelper }

  TdxCalendarDateCellAccessibilityHelper = class(TdxCalendarElementAccessibilityHelper)
  strict private
    FSelectionItemProvider: TdxCalendarDateCellSelectionItemProvider;
  protected
    procedure DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID); override;
    ...
    function GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
    function GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties; override;
    function IsSupportedPattern(APatternID: Integer; out AProvider: IUnknown;
      AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean; override;
  public
    constructor Create(AOwnerObject: TObject; AParent: TcxAccessibilityHelper); override;
    destructor Destroy; override;
  end;
  ...
  { TdxCalendarDateHeaderCellAccessibilityHelper }

  TdxCalendarDateHeaderCellAccessibilityHelper = class(TdxCalendarButtonAccessibilityHelper)
  protected
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;
    function GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
  end;
  ...
  protected
    ...
    procedure ChangeSelectedAccesibleObject;
    procedure CreateElements; override;
    procedure DestroyElements; override;
    function DoCalculateSize: TSize; override;
    function GetCurrentTableViewInfo: TcxCalendarDateTableViewInfo; virtual;
  ...
  public
    ...
    procedure ChangeSelectDate(AValue: Double; ARepositionVisibleDates: Boolean; AAnimated: Boolean); virtual;
  ...
  protected
    ...
    procedure DestroyElements; override;
    function DoCalculateSize: TSize; override;
    function GetCurrentTableViewInfo: TcxCalendarDateTableViewInfo; override;
  ...
  public
    ...
    procedure ChangeSelectDate(AValue: Double; ARepositionVisibleDates: Boolean; AAnimated: Boolean); override;
    property TableViewInfo [AView: TcxCalendarModernDateViewStyle]: TcxCalendarDateTableViewInfo read GetTableViewInfo;
  end;

  { TdxCalendarHeaderAccessibilityHelper }

  TdxCalendarHeaderAccessibilityHelper = class(TdxCalendarElementAccessibilityHelper)
  protected
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;
    function GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
    function GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties; override;
  ...
  { TdxCalendarClockAccessibilityHelper }

  TdxCalendarClockAccessibilityHelper = class(TdxCalendarElementAccessibilityHelper)
  protected
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;
  end;
  ...
  protected
    ...
    function DoCalculateSize: TSize; override;
    procedure DoRightToLeftConversion(const ABounds: TRect); override;
    function GetIAccessibilityHelper: IcxAccessibilityHelper; override;
    procedure InitControls; override;
    procedure InitializeVisibleElements; override;
  public
    constructor Create(ACalendar: TcxCustomCalendar); override;
  ...
  protected
    FAccessibleObjects: TList<TdxCalendarElementAccessibilityHelper>;
  ...
  protected
    ...
    procedure DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect); override;
    procedure Paint; override;
    ...
    function GetValidSelectDate: Double;
...
uses
  ...
  cxVariants, dxDPIAwareUtils, dxCoreGraphics;

const
  dxThisUnitName = 'cxCalendar';
...
function VarIsNullDate(const AValue: Variant): Boolean;
begin
  Result := (VarIsDate(AValue) or VarIsNumericEx(AValue)) and (Integer(AValue) = NullDate);
end;
...
constructor TdxCalendarElementAccessibilityHelper.Create(AOwnerObject: TObject; AParent: TcxAccessibilityHelper);
begin
  inherited Create(AOwnerObject);
  FParent :=  AParent;
  FIAccessibilityChildren := TInterfaceList.Create;
  FLocalId := ViewInfo.Calendar.ViewInfo.FAccessibleObjects.Count + 1;
  ViewInfo.Calendar.ViewInfo.FAccessibleObjects.Add(Self);
end;

destructor TdxCalendarElementAccessibilityHelper.Destroy;
begin
  FreeAndNil(FIAccessibilityChildren);
  inherited;
end;

function TdxCalendarElementAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetOrigin(ViewInfo.Bounds, ViewInfo.Calendar.ClientToScreen(ViewInfo.Bounds.TopLeft));
end;

function TdxCalendarElementAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

procedure TdxCalendarElementAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  ViewInfo.Click;
end;

function TdxCalendarElementAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := IcxAccessibilityHelper(FIAccessibilityChildren[AIndex]).GetHelper;
end;

function TdxCalendarElementAccessibilityHelper.GetChildCount: Integer;
begin
  ViewInfo.PopulateIAccessibilityChildren(FIAccessibilityChildren);
  Result := FIAccessibilityChildren.Count;
end;

function TdxCalendarElementAccessibilityHelper.GetLocalId(AChildID: TcxAccessibleSimpleChildElementID = 0): Integer;
begin
  Result := FlocalId;
end;

function TdxCalendarElementAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := ViewInfo.Calendar.Handle;
end;

function TdxCalendarElementAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := FParent;
end;

function TdxCalendarElementAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_CLIENT;
end;

function TdxCalendarElementAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
...
function TdxCalendarElementAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
end;

function TdxCalendarElementAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

procedure TdxCalendarElementAccessibilityHelper.OwnerObjectDestroyed;
begin
  if (ViewInfo.Calendar.ViewInfo <> nil) and (ViewInfo.Calendar.ViewInfo.FAccessibleObjects <> nil) then
    ViewInfo.Calendar.ViewInfo.FAccessibleObjects[FLocalId - 1] := nil;
  inherited;
end;

function TdxCalendarElementAccessibilityHelper.GetViewInfo: TcxCalendarElementViewInfo;
begin
  Result := TcxCalendarElementViewInfo(OwnerObject);
end;

{ TdxCalendarButtonAccessibilityHelper }

function TdxCalendarButtonAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Click';
end;

function TdxCalendarButtonAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxCalendarButtonAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := inherited + [aopDefaultAction];
end;
...
function TcxCalendarElementViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := TdxCalendarElementAccessibilityHelper;
end;
...
function TcxCalendarElementViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
procedure TcxCalendarElementViewInfo.PopulateIAccessibilityChildren(IAccessibilityChildren: TInterfaceList);
var
  I: Integer;
  ACalendarElementViewInfo: TcxCalendarElementViewInfo;
begin
  IAccessibilityChildren.Clear;
  for I := 0 to VisibleElements.Count - 1 do
  begin
    ACalendarElementViewInfo := TcxCalendarElementViewInfo(VisibleElements[I]);
    if ACalendarElementViewInfo.IAccessibilityHelper = nil then
      ACalendarElementViewInfo.FIAccessibilityHelper := ACalendarElementViewInfo.GetAccessibilityHelperClass.Create(
        VisibleElements[I], IAccessibilityHelper.GetHelper);

    IAccessibilityChildren.Add(ACalendarElementViewInfo.IAccessibilityHelper);
  end;
end;
...
procedure TdxCalendarArrowAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  TcxCalendarArrowViewInfo(ViewInfo).DoAction;
end;

function TdxCalendarArrowAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if TcxCalendarArrowViewInfo(ViewInfo).Direction = adLeft then
    Result := 'Back'
  else
    Result := 'Next';
end;
...
function TcxCalendarArrowViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := TdxCalendarArrowAccessibilityHelper;
end;
...
constructor TdxCalendarDateCellSelectionItemProvider.Create(AViewInfo: TcxCalendarDateCellViewInfo);
begin
  FViewInfo := AViewInfo;
end;

procedure TdxCalendarDateCellSelectionItemProvider.DoSelect;
begin
  TcxCalendarDateCellViewInfo(FViewInfo).Select;
end;

function TdxCalendarDateCellSelectionItemProvider.IsSelected: Boolean;
begin
  Result := TcxCalendarDateCellViewInfo(FViewInfo).IsSelected;
end;

{ TdxCalendarDateCellAccessibilityHelper }

constructor TdxCalendarDateCellAccessibilityHelper.Create(AOwnerObject: TObject; AParent: TcxAccessibilityHelper);
begin
  inherited Create(AOwnerObject, AParent);
  FSelectionItemProvider := TdxCalendarDateCellSelectionItemProvider.Create(TcxCalendarDateCellViewInfo(ViewInfo));
end;

destructor TdxCalendarDateCellAccessibilityHelper.Destroy;
begin
  FreeAndNil(FSelectionItemProvider);
  inherited;
end;

procedure TdxCalendarDateCellAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  ViewInfo.DblClick;
end;

procedure TdxCalendarDateCellAccessibilityHelper.DoSelect(AFlags: Integer; AChildID: TcxAccessibleSimpleChildElementID);
begin
  TcxCalendarDateCellViewInfo(ViewInfo).Select;
end;

function TdxCalendarDateCellAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Double click';
end;

function TdxCalendarDateCellAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := TcxCalendarDateCellViewInfo(ViewInfo).GetText;
end;

function TdxCalendarDateCellAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_CELL;
end;

function TdxCalendarDateCellAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited or cxSTATE_SYSTEM_SELECTABLE or cxSTATE_SYSTEM_FOCUSABLE;
  if TcxCalendarDateCellViewInfo(ViewInfo).IsSelected then
    Result := Result or cxSTATE_SYSTEM_SELECTED or cxSTATE_SYSTEM_FOCUSED;
  if not TcxCalendarDateCellViewInfo(ViewInfo).IsEnabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
end;

function TdxCalendarDateCellAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := inherited + [aopSelect, aopDefaultAction];
end;

function TdxCalendarDateCellAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IUnknown;
  AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean;
begin
  Result := APatternID = dxUIA_SelectionItemPatternId;
  if Result then
    AProvider := FSelectionItemProvider;
end;
...
function TcxCalendarDateCellViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := TdxCalendarDateCellAccessibilityHelper;
end;
...
procedure TcxCalendarDateCellViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  function IsValidDate(AValue: Double): Boolean; 
  begin
    Result := (AValue <> InvalidDate) and cxGetLocalCalendar.IsValidDate(AValue);
  end;

var
  AStates: TcxCalendarElementStates;
begin
  if Calendar.CanShowDate(Value) and IsValidDate(Value) then
  begin
    ACanvas.SaveState;
    try
      ACanvas.Font := Font;
...
begin
  Result := Calendar.GetValidSelectDate;
end;
...
var
  ASize: TSize;
begin
  ASize := TdxTextMeasurer.TextSizeTO(Font, GetText);
  Result := cxRectCenter(Bounds, 3 * TdxTextMeasurer.TextWidthTO(Font, '0'), ASize.cy);
...
begin
  ...
  if Assigned(Calendar.FOnGetDayOfWeekState) then
    Calendar.FOnGetDayOfWeekState(Calendar, dxDayOfWeek(FDate), CalendarElementStateToCustomDrawState(AStates), ACanvas.Font, AColor);
  if Assigned(Calendar.FOnGetDayState) then
    Calendar.FOnGetDayState(Calendar, FDate, CalendarElementStateToCustomDrawState(AStates), ACanvas.Font, AColor);
...
begin
  ...
  if FDate = Date then
    ...
    if cxIsWeekendDay(FDate) then
      Include(Result, cesHighlighted)
    else
...
begin
  Result := cxRectCenter(Bounds, TdxTextMeasurer.TextSizeDT(Font, '00'));
end;
...
function cxSizeInflate(const ASize: TSize; const AContentOffset: TRect): TSize;
begin
  Result.cx := ASize.cx + cxMarginsWidth(AContentOffset);
  Result.cy := ASize.cy + cxMarginsHeight(AContentOffset);
end;

function TcxCalendarModernDayCellViewInfo.GetMinSize: TSize;
begin
  Result := dxGetTouchableSize(cxSizeInflate((ViewInfo as TcxCalendarModernViewInfo).FMaxDayOfWeekNameSize, GetContentOffset), Calendar.ScaleFactor);
end;
...
begin
  Result := cxRectCenter(Bounds, TdxTextMeasurer.TextSizeDT(Font, GetText));
end;
...
begin
  Result := dxGetTouchableSize(cxSizeInflate(TdxTextMeasurer.TextSizeDT(Font, 'Feb'), GetContentOffset), Calendar.ScaleFactor);
end;
...
begin
  Result := dxGetTouchableSize(cxSizeInflate(TdxTextMeasurer.TextSizeDT(Font, '9999'), GetContentOffset), Calendar.ScaleFactor);
end;

function TcxCalendarModernYearCellViewInfo.GetText: string;
begin
  if Value = InvalidDate then
    Result := ''
  else
...
begin
  Result := TdxTextMeasurer.TextSizeDT(Font, '9999-');
  Result.cy := Result.cy * 2;
  Result := dxGetTouchableSize(cxSizeInflate(Result, GetContentOffset), Calendar.ScaleFactor);
end;
...
begin
  ...
  begin
    P1 := Point(ATextRect.Left, Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight - 1);
    P2 := Point(ATextRect.Right - 1, Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight - 1);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        P2 := Point(AWeekNumbersDelimiterPos, Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight - 1);
        InternalPolyLine(ACanvas, [P1, P2], clBtnShadow, True);
        P1 := P2;
        P2 := Point(AWeekNumbersDelimiterPos + 1, Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight - 1);
      ...
      begin
        ...
        InternalPolyLine(ACanvas, [Point(AWeekNumbersDelimiterPos, Bounds.Bottom - 2),
          Point(AWeekNumbersDelimiterPos, Bounds.Bottom - 1)], Calendar.BackgroundColor, True);
      end;
    ...
    begin
      ...
      begin
        ...
        InternalPolyLine(ACanvas, [P1, P2], clBtnShadow, True);
        P1 := P2;
        P2 := Point(AWeekNumbersDelimiterPos - 2, Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight - 1);
        InternalPolyLine(ACanvas, [P2, P2], clBtnShadow, True);
...
begin
  ...
  FWeekNumberWidth := TdxTextMeasurer.TextWidthTO(Font, '99', MulDiv(Font.Size, 2, 3));
end;
...
function TcxCalendarModernMonthTableViewInfo.GetDayOfWeekName(ADay: TDay;
  AFontCharset: TFontCharset): string;
begin
  Result := cxGetDayOfWeekName(ADay, AFontCharset); //FormatDateTime('ddd', Ord(ADay) + 1);
end;
...
function TdxCalendarDateHeaderCellAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := TcxCalendarDateHeaderCellViewInfo(ViewInfo).GetText;
end;

function TdxCalendarDateHeaderCellAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited;
  if not TcxCalendarDateHeaderCellViewInfo(ViewInfo).IsEnabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
end;

{ TcxCalendarDateHeaderCellViewInfo }
...
begin
  Result := TdxTextMeasurer.TextSizeDT(Font, GetText);
end;
...
function TcxCalendarDateHeaderCellViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := TdxCalendarDateHeaderCellAccessibilityHelper;
end;
...
begin
  ...
  R1 := cxRectCenterVertically(Bounds, AArrowSize.cy);
  R1.Right := R1.Left + AArrowSize.cx;
  FArrowsViewInfo[adLeft].Bounds := R1;
...
begin
  ...
  begin
    ...
    if LookAndFeelPainter.LookAndFeelStyle = lfsNative then
      dxDrawThemeBackground(ACanvas.Handle, totHeader, Calendar.ScaleFactor, Bounds, HP_HEADERITEMLEFT, HIS_NORMAL)
    else
...
begin
  ...
  begin
    AMonthNameWidth := cxMeasureCanvas.TextWidth(cxGetLocalMonthName(cxGetLocalCalendar.AddMonths(Calendar.FirstDate, I), cxGetLocalCalendar));
    if AMonthNameWidth > AMaxMonthNameWidth then
      AMaxMonthNameWidth := AMonthNameWidth;
    AYearNameWidth := cxMeasureCanvas.TextWidth(cxGetLocalYear(cxGetLocalCalendar.AddYears(Calendar.FirstDate, I), cxGetLocalCalendar));
...
function TcxCalendarModernDateHeadersViewInfo.DoCalculateSize: TSize;
var
  H: Integer;
begin
  Result := inherited DoCalculateSize;
  H := MonthHeaderViewInfo.TextCellViewInfo.Size.cy + cxMarginsHeight(LookAndFeelPainter.GetModernCalendarHeaderTextOffsets);
...
procedure TcxCalendarDateViewInfo.ChangeSelectedAccesibleObject;
var
  I: Integer;
  AViewInfo: TcxCalendarDateCellViewInfo;
begin
  if (IAccessibilityHelper <> nil) and (IAccessibilityHelper.GetHelper.ChildCount > 0) then
    for I := 0 to GetCurrentTableViewInfo.IAccessibilityHelper.GetHelper.ChildCount - 1 do
    begin
      AViewInfo := GetCurrentTableViewInfo.DateCells[I];
      if AViewInfo.IsSelected then
        NotifyWinEvent(EVENT_OBJECT_FOCUS, Calendar.Handle,
          TdxCalendarElementAccessibilityHelper(AViewInfo.IAccessibilityHelper).FLocalId, CHILDID_SELF);
    end;
end;
...
function TcxCalendarDateViewInfo.GetCurrentTableViewInfo: TcxCalendarDateTableViewInfo;
begin
  Result := MonthTableViewInfo;
end;
...
begin
  inherited;
  FSideWidth := 2 * TdxTextMeasurer.TextWidthTO(Font, '0');
end;
...
begin
  ...
  try
    ...
    Calendar.DrawOpaqueBackground(ACanvas, Bounds);
...
function TdxCalendarHeaderAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := TcxCalendarHeaderViewInfo(ViewInfo).GetText;
end;

function TdxCalendarHeaderAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_STATICTEXT;
end;

function TdxCalendarHeaderAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation];
end;
...
begin
  FTextRect := cxRectCenter(Bounds, FTextSize);
  inherited CalculateBounds;
end;

function TcxCalendarHeaderViewInfo.DoCalculateSize: TSize;
begin
  FTextSize := TdxTextMeasurer.TextSizeDT(Font, GetText);
  Result := FTextSize;
end;
...
begin
  Result := cxDateToLocalFormatStr(Calendar.SelectDate + cxSign(Calendar.SelectDate) * Calendar.Time);
end;
...
function TcxCalendarHeaderViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := TdxCalendarHeaderAccessibilityHelper;
end;
...
function TcxCalendarModernHeaderViewInfo.DoCalculateSize: TSize;
begin
  Result := inherited DoCalculateSize;
  Inc(Result.cy, cxMarginsHeight(LookAndFeelPainter.GetModernCalendarHeaderTextOffsets));
end;
...
function TdxCalendarClockAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'TdxCalendarClockAccessibilityHelper';
end;
...
function TcxCalendarClockViewInfo.GetAccessibilityHelperClass: TdxCalendarElementAccessibilityHelperClass;
begin
  Result := inherited;// TdxCalendarClockAccessibilityHelper;
end;
...
function TcxCalendarTimeEditViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  Result := TcxCustomEditAccess(FTimeEdit).IAccessibilityHelper;
end;
...
begin
  ...
  cxMeasureCanvas.Font := Font;
  Result := FTimeEdit.ActiveProperties.GetEditSize(cxMeasureCanvas, FTimeEdit.Style,
    True, 0, AEditSizeProperties).cx + cxMeasureCanvas.TextWidth('0');
end;
...
begin
  ...
  AButtonVOffset := (ViewInfo.FButtonsAreaHeight - 1 - ViewInfo.FButtonsHeight) div 2;
...
begin
  R := Bounds;
  ATimeEditOffset := (ViewInfo.FButtonsAreaHeight - FTimeEditInfo.Size.cy) div 2;
...
begin
  Result.cx := Max(FClockInfo.Size.cx, FTimeEditInfo.Size.cx);
  Result.cy := FClockInfo.Size.cy + ViewInfo.FButtonsAreaHeight;
end;
...
begin
  Result :=  TdxTextMeasurer.TextWidthTO(Font, FButton.Caption);
end;

function TcxCalendarButtonViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin

  Result := TcxCustomButtonAccess(FButton).IAccessibilityHelper;
end;
...
procedure TcxCalendarButtonsViewInfo.DoCalculateBounds;
  ...
  begin
    Result := cxRectBounds(
      ...
      Calendar.ClientHeight - ViewInfo.FButtonsAreaHeight + ViewInfo.FButtonsOffset,
  ...
  begin
    AButtonsTop := Calendar.Height - ViewInfo.FButtonsAreaHeight + 1 +
      (ViewInfo.FButtonsAreaHeight - 1 - ViewInfo.FButtonsHeight) div 2;
    AButtonsOffset := MulDiv(ViewInfo.FFontSize, 5, 4);
...
begin
  ...
  Result.cy := ViewInfo.FButtonsAreaHeight;
end;
...
constructor TcxCalendarViewInfo.Create(ACalendar: TcxCustomCalendar);
begin
  FAccessibleObjects := TList<TdxCalendarElementAccessibilityHelper>.Create;
  inherited;
end;

destructor TcxCalendarViewInfo.Destroy;
begin
...
begin
  ASize := TdxTextMeasurer.TextSizeTO(Font, '0');
  ATextHeight := ASize.cy;
  FFontSize := GetFontSize(Font);
  FColWidth := dxGetTouchableSize(3 * ASize.cx, Calendar.ScaleFactor);
  FRowHeight := dxGetTouchableSize(ATextHeight + 2, Calendar.ScaleFactor);
  FHeaderHeight := FRowHeight + 3;
  if (Kind = ckDate) and (FCalendar.LookAndFeelPainter.LookAndFeelStyle = lfsOffice11) then
    Dec(FHeaderHeight);

  FButtonsOffset := FFontSize div 2;
  FButtonsHeight := dxGetTouchableSize(MulDiv(ATextHeight, 20, 13) + 1, Calendar.ScaleFactor);

  if IsTimeVisible then
  begin
    FButtonsAreaHeight := MulDiv(FFontSize, 5, 4) + 1;
    if not Odd(FButtonsAreaHeight) then
      Inc(FButtonsAreaHeight);
    Inc(FButtonsAreaHeight, FButtonsHeight);
  end
  else
    FButtonsAreaHeight := FButtonsOffset + (FButtonsHeight - 1) + MulDiv(FFontSize, 3, 4);
...
begin
  ...
  if IsHeaderVisible then
    Result.cy := Result.cy + FHeaderHeight + FCalendar.GetMonthCalendarOffset.Y + FCalendar.GetButtonsRegionOffset
  else
  ...
  begin
    ...
    Result.cx := Result.cx + FCalendar.GetMonthCalendarOffset.X * 2 + FColWidth + FTimeViewInfo.Size.cx;
  end;
...
function TcxCalendarViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := GetAccessibilityHelperClass.Create(Self, nil);
...
begin
  ...
  begin
    ...
    R.Bottom := R.Top + Max(FHeaderHeight, FHeaderViewInfo.Size.cy);
  ...
  begin
    R := Bounds;
    R.Top := R.Bottom - FButtonsAreaHeight;
...
begin
  ...
  if IsHeaderVisible then
    Result.cy := Result.cy + Max(FHeaderHeight, FHeaderViewInfo.Size.cy) + GetHeaderOffset;
...
begin
  if IsTimeVisible then
    AInterval := MulDiv(FFontSize, 7, 4)
  else
    AInterval := MulDiv(FFontSize, 3, 8);
  Result := Rect(AInterval, FFontSize, AInterval, AInterval);
end;

function TcxCalendarModernViewInfo.GetDateTimeInfoOffset: Integer;
begin
  Result :=  MulDiv(FFontSize, 7, 4);
end;
...
var
  ASelectDate: Double;
begin
  ASelectDate := Calendar.GetValidSelectDate;
  case DateViewInfo.ViewStyle of
    cvdsMonth: Result := cxGetLocalMonthYear(ASelectDate, cxGetLocalCalendar);
    cvdsYear: Result := cxGetLocalYear(ASelectDate, cxGetLocalCalendar);
    cvdsDecade: Result := GetYearsGroup(ASelectDate, 10);
  else // cvdsCentury
    Result := GetYearsGroup(ASelectDate, 100);
  end;
end;
...
procedure TcxCalendarModernViewInfo.Initialize;
var
  I: Integer;
  ASize: TSize;
begin
  inherited Initialize;
  FMaxDayOfWeekNameSize := TdxTextMeasurer.TextSizeDT(Font, 'Wed');
  for I := Low(TdxDayOfWeekVCL) to High(TdxDayOfWeekVCL) do
  begin
    ASize := TdxTextMeasurer.TextSizeDT(Font, FormatDateTime('ddd', I));
    if ASize.cx > FMaxDayOfWeekNameSize.cx then
      FMaxDayOfWeekNameSize := ASize;
  end;
end;
...
procedure TcxCalendarPainter.DrawHeaderText(ACanvas: TcxCanvas; AViewInfo: TcxCalendarHeaderViewInfo);
begin
end;

function TcxCalendarPainter.GetCellTextColor(ADrawStates: TcxCalendarElementStates): TColor;
begin
  if cesSelected in ADrawStates then
    Result := LookAndFeelPainter.DefaultDateNavigatorSelectionTextColor
  else if cesDisabled in ADrawStates then
    Result := LookAndFeelPainter.DefaultDateNavigatorInactiveTextColor
  else if cesMarked in ADrawStates then
    Result := LookAndFeelPainter.DefaultDateNavigatorTodayTextColor
  else if cesHighlighted in ADrawStates then
    Result := LookAndFeelPainter.DefaultDateNavigatorWeekendTextColor
  else
...
function TcxCalendarModernPainter.GetCellTextColor(ADrawStates: TcxCalendarElementStates): TColor;
begin
...
function TcxCustomCalendar.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  Result := ViewInfo.IAccessibilityHelper;
end;
...
procedure TcxCustomCalendar.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
  ...
  begin
    ...
    if (AObjectID = OBJID_CLIENT) or (AObjectID = OBJID_WINDOW) then
      ...
      if (AObjectID > 0) and (AObjectID <= Cardinal(ViewInfo.FAccessibleObjects.Count)) then
        Message.Result := WMGetObjectResultFromIAccessibilityHelper(Message, ViewInfo.FAccessibleObjects.Items[AObjectID - 1])
      else
        inherited;
  end
  else
...
procedure TcxCustomCalendar.DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
  ACanvas.FillRect(ARect, BackgroundColor);
end;
...
begin
  Result := FViewInfo.FFontSize div 2;
end;
...
begin
  Result.X := MulDiv(FViewInfo.FFontSize, 3, 4);
  Result.Y := Result.X;
end;
...
function TcxCustomCalendar.GetValidSelectDate: Double;
begin
  if SelectDate = NullDate then
    Result := Date
  else
    Result := SelectDate;
end;
...
procedure TcxCustomCalendar.InternalSetSelectDate(Value: Double; ARepositionVisibleDates: Boolean);
begin
...
begin
  ...
  begin
    if Kind = ckDateTime then
      ...
      Result := (ShowTime and (dxTimeOf(ADate) <> 0) and (InputKind = ikStandard)) or
        (EditFormat <> '');
  end
  else
    Result := not((Kind = ckDate) and not ShowTime) or (DisplayFormat <> '');
end;
...
function TcxCustomDateEditProperties.GetEmptyDisplayValue(AEditFocused: Boolean): string;
var
...
begin
  ...
  begin
    ...
    ATimeStringLength := cxFormatController.GetDateTimeStandardMaskStringLength(cxFormatController.TimeFormatInfo);
...
function TcxCustomDateEditProperties.GetTimeZoneInfo(APos: Integer; out AInfo: TcxTimeEditZoneInfo): Boolean;
const
  ATimeZoneKindMap: array[TcxDateTimeFormatItemKind] of TcxTimeEditZoneKind =
    (tzHour, tzHour, tzHour, tzHour, tzHour, tzMin, tzSec, tzHour, tzTimeSuffix, tzHour, tzHour);
var
...
begin
  ...
  if (Kind <> ckDateTime) or (InputKind <> ikMask) or
    cxFormatController.AssignedStandardDateTimeEditMask then
      Exit;
  Result := cxFormatController.GetDateTimeFormatItemStandardMaskInfo(cxFormatController.DateTimeFormatInfo, APos, AItemInfo);
...
function TcxCustomDateEditProperties.IsDisplayValueValid(var DisplayValue: TcxEditValue; AEditFocused: Boolean): Boolean;
var
  ADate: TDateTime;
begin
...
function TcxCustomDateEditProperties.IsEditValueValid(var EditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
var
  AValue: TcxEditValue;
begin
      ...
      begin
        ...
        if AEditFocused then
          ...
          if DisplayFormat <> '' then
            ...
            Result := DateTimeToTextEx(AValue, False, Kind = ckDateTime);
...
begin
  ...
  if AEditFocused and IsMasked and cxFormatController.IsStandardMaskValid and (EditFormat = '') then
...
procedure TcxCustomDateEditProperties.ValidateDisplayValue(
  ...
  procedure ConvertToDate(const ADisplayValue: TcxEditValue; out ADate: TDateTime; out AError: Boolean);
  begin
    try
  ...
  function GetDate(var ADisplayValue: TcxEditValue; out AError: Boolean; out AErrorText: TCaption): TDateTime;
  var
    ...
    try
      ...
      begin
        if (ADisplayValue <> GetEmptyDisplayValue(True)) and not AError then
          ADisplayValue := DateToDisplayValue(Result);
  ...
  procedure ValidateEmptyDisplayValue(AIsNullDate: Boolean);
    ...
    function IsEditFormatContainsTimePart: Boolean;
    var
    ...
    begin
      ...
      begin
        ...
        try
          GetDateTimeFormatInfo(EditFormat, AFormatInfo);
          Result := AFormatInfo.DefinedItems[dtikHour] or
            AFormatInfo.DefinedItems[dtikMin] or
            AFormatInfo.DefinedItems[dtikSec] or
            AFormatInfo.DefinedItems[dtikMSec];
        finally
          AFormatInfo.Free;
        end;
      end;
    ...
    begin
      Result := not AIsNullDate and (Kind = ckDate) and not IsEditFormatContainsTimePart and
...
procedure TcxCustomDateEdit.DateChange(Sender: TObject);
  ...
  begin
    ...
    Result := dxDateOf(ADateTime) + EncodeTime(H, M, S, 0);
  end;
...
begin
  ...
  try
    ...
    if ((Date <> ADate) or not InternalCompareString(DisplayText, VarToStr(ADisplayValue), True)) and DoEditing then
    begin
...
function TcxCustomDateEdit.GetRecognizableDisplayValue(ADate: TDateTime): TcxEditValue;
...
begin
  ...
  BeginUserAction;
  try
    if ActiveProperties.IsValueBoundDefined(evbMin) and (Date < ActiveProperties.MinValue) then
      Date := ActiveProperties.MinValue
    else
      if ActiveProperties.IsValueBoundDefined(evbMax) and (Date > ActiveProperties.MaxValue) then
